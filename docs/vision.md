...
| Módulo / Actor Interno         | Tipo de Interacción | Descripción |
|--------------------------------|---------------------|-------------|
| Módulo Carga de Horas          | API interna         | Consulta costos por perfil/mes para combinar con las horas registradas y calcular costos de mano de obra en sus reportes. |
| Equipo de Finanzas / Contador  | API y herramientas  | Configura y revisa costos por hora por perfil/mes, y consulta costos consolidados por proyecto para análisis básico de rentabilidad. |
| Responsable de Proyectos       | Reportes (vía UI)   | Visualiza costos de mano de obra por proyecto y por recurso/mes como insumo para decisiones de seguimiento de proyectos. |

### Módulos que este Módulo Consume
| Módulo / Sistema Externo      | Tipo de Interacción | Descripción |
|-------------------------------|---------------------|-------------|
| Módulo Carga de Horas         | API interna         | Lee las entradas de horas registradas por persona/tarea/proyecto para calcular el costo de mano de obra. Esta información no se persiste localmente en Finanzas. |
| API de Proyectos              | API externa         | Obtiene en forma dinámica la lista de proyectos y sus identificadores para agrupar las horas y los costos; los proyectos no se persisten localmente. |
...
| API de Roles/Perfiles         | API externa         | Consulta los perfiles/roles válidos y sus IDs externos; Finanzas solo persiste el costo por hora mensual asociado a cada perfil. |
...
- Servicio de Carga de Horas (módulo del otro squad) para obtener horas registradas por proyecto, tarea y recurso.
